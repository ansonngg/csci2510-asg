...
SearchSets:
	...
	; *** fill your code here ***
	mov CL, BlockSize
	shr EAX, CL ; EAX contains the block address
	mov EBX, EAX
	and EBX, 31 ; EBX indicates the set which the data is mapped to
	shl EBX, 4 ; EBX contains the address of the first block in the corresponding set
	push EBP ; Save the base address of CacheTags
	add EBP, EBX
	mov CL, 5
	shr EAX, CL ; EAX contains the tag
	mov ECX, 0
	jmp SearchTags
...
CacheMiss: ; Handle the cache miss by FIFO replacement algorithm
	...
	; *** fill your code here ***
	push ESI ; Save the base address of CacheTimes
	add ESI, EBX
	mov ECX, 0
	mov EBX, ECX ; EBX stores the index of the block whose timestamp is the smallest
	mov EDX, [ESI + 4 * ECX] ; EDX stores the smallest timestamp
my_repeat:
	inc ECX
	cmp ECX, SetSize
	je done
